...
func Test_TstFile(t *testing.T) {
	t.Run("OSFile", func(t *testing.T) { TstFile(t, createOSFile) })
	t.Run("KitFile", func(t *testing.T) { TstFile(t, createKitFile) })
}

func Test_TstDirectory(t *testing.T) {
	// --- Given ---
	dir := tstDirOS(t)
	osDir := must.Value(os.OpenRoot(dir))
	kitDir := tstDirMem()

	// --- Then ---
	t.Run("OSDir", func(t *testing.T) { TstDirectory(t, dir, osDir.FS()) })
	t.Run("KitDir", func(t *testing.T) { TstDirectory(t, dir, kitDir.FS()) })
}

// TstFile performs tests on instances of a file created by the create function.
func TstFile(t *testing.T, create fileCreator) {
	t.Helper()

	t.Run("Write - O_APPEND", func(t *testing.T) {
		// --- Given ---
		fil := create(t, os.O_RDWR|os.O_APPEND, []byte{0, 1, 2})
	...
	t.Run("Write - partial override and append", func(t *testing.T) {
		// --- Given ---
		fil := create(t, os.O_RDWR, []byte{0, 1, 2})
		iokit.Seek(fil, 1, io.SeekStart)
	...
	t.Run("Write - override middle", func(t *testing.T) {
		// --- Given ---
		fil := create(t, os.O_RDWR, []byte{0, 1, 2, 3})
		iokit.Seek(fil, 1, io.SeekStart)
	...
	t.Run("Read - with a small buffer", func(t *testing.T) {
		// --- Given ---
		fil := create(t, os.O_RDWR, []byte{0, 1, 2, 3, 4})
		dst := make([]byte, 3)
	...
	t.Run("ReadFrom - overwrite", func(t *testing.T) {
		// --- Given ---
		fil := create(t, os.O_RDWR, []byte("abc"))
		rdr := bytes.NewBuffer([]byte{0, 1, 2})
	...
	t.Run("Seek - error - SeekEnd negative error", func(t *testing.T) {
		// --- Given ---
		fil := create(t, os.O_RDWR, []byte{0, 1, 2})
		...
		assert.Equal(t, "file.txt", filepath.Base(e.Path))
		assert.Equal(t, int64(0), have)
		assert.Equal(t, []byte{0, 1, 2}, must.Value(io.ReadAll(fil)))
	})
	...
	t.Run("Truncate - beyond length with O_APPEND", func(t *testing.T) {
		// --- Given ---
		fil := create(t, os.O_RDWR|os.O_APPEND, []byte{0, 1, 2, 3})
	...
	t.Run("Truncate - beyond capacity and write", func(t *testing.T) {
		...
		fil := create(t, os.O_RDWR, content)
	...
	t.Run("Truncate - error - seek than negative offset", func(t *testing.T) {
		// --- Given ---
		fil := create(t, os.O_RDWR, []byte{0, 1, 2})
		iokit.Seek(fil, 1, io.SeekStart)
	...
	t.Run("Stat - O_RDWR", func(t *testing.T) {
		// --- Given ---
		fil := create(t, os.O_RDWR, []byte{0, 1, 2})
		...
		assert.Equal(t, fs.FileMode(0600), fi.Mode())
	})

	t.Run("Stat - O_RDWR and O_APPEND", func(t *testing.T) {
		// --- Given ---
		fil := create(t, os.O_RDWR|os.O_APPEND, []byte{0, 1, 2})
	...
	t.Run("Stat - size changed", func(t *testing.T) {
		// --- Given ---
		fil := create(t, os.O_RDWR|os.O_APPEND, []byte{0, 1, 2})
		must.Value(fil.Write([]byte{3, 4}))
...
// TstDirectory performs tests on the given root filesystem.
func TstDirectory(t *testing.T, pth string, root fs.FS) {
	t.Helper()

	t.Run("ReadFile - read an existing file", func(t *testing.T) {
		// --- When ---
		have, err := fs.ReadFile(root, "file1")
	...
	t.Run("ReadFile - error - read not existing entry", func(t *testing.T) {
		// --- When ---
		have, err := fs.ReadFile(root, "not-existing")
	...
	t.Run("ReadFile - error - read a directory", func(t *testing.T) {
		// --- When ---
		have, err := fs.ReadFile(root, "sub")

		// --- Then ---
		var e *fs.PathError
		assert.ErrorAs(t, &e, err)
		assert.Equal(t, "read", e.Op)
		assert.Equal(t, "sub", strings.TrimPrefix(e.Path, pth+"/"))
		assert.Equal(t, syscall.EISDIR, e.Err)
		assert.Len(t, 0, have)
	})

	t.Run("Stat - an existing file", func(t *testing.T) {
		// --- When ---
		have, err := fs.Stat(root, "file1")

		// --- Then ---
		assert.NoError(t, err)
		assert.Equal(t, "file1", have.Name())
		assert.Equal(t, int64(5), have.Size())
		assert.Equal(t, fs.FileMode(0600), have.Mode())
		assert.False(t, have.IsDir())
	})

	t.Run("Stat - an existing directory", func(t *testing.T) {
		// --- When ---
		have, err := fs.Stat(root, "sub")
	...
	t.Run("Stat - error - not existing file", func(t *testing.T) {
		// --- When ---
		have, err := fs.Stat(root, "not-existing")
	...
	t.Run("ReadDir - an existing directory", func(t *testing.T) {
		// --- When ---
		have, err := fs.ReadDir(root, "sub")
	...
	t.Run("ReadDir - an existing sub directory", func(t *testing.T) {
		// --- When ---
		have, err := fs.ReadDir(root, "sub/sub2")
	...
	t.Run("ReadDir - error - not existing directory", func(t *testing.T) {
		// --- When ---
		have, err := fs.ReadDir(root, "not-existing")
	...
	t.Run("ReadDir - error - when the name is a file", func(t *testing.T) {
		// --- When ---
		have, err := fs.ReadDir(root, "file0")

		// --- Then ---
		var e *fs.PathError
		assert.ErrorAs(t, &e, err)
		assert.Equal(t, "readdirent", e.Op)
		assert.Equal(t, "file0", strings.TrimPrefix(e.Path, pth+"/"))
		assert.Equal(t, syscall.ENOTDIR, e.Err)
		assert.Len(t, 0, have)
	})

	t.Run("Open - open existing file", func(t *testing.T) {
		// --- When ---
		have, err := root.Open("file1")
	...
	t.Run("Open - error - open not existing file", func(t *testing.T) {
		// --- When ---
		have, err := root.Open("not-existing")
	...
	t.Run("Open - deep open file", func(t *testing.T) {
		// --- When ---
		have, err := root.Open("sub/file3")
	...
	t.Run("Open - deep open directory", func(t *testing.T) {
		// --- When ---
		have, err := root.Open("sub/sub2")

		// --- Then ---
		assert.NoError(t, err)
		assert.True(t, must.Value(have.Stat()).IsDir())
		assert.Equal(t, "sub2", must.Value(have.Stat()).Name())
	})
}
